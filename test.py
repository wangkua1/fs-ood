...
parser.add_argument('--model.use_support_stats', type=int, default=0)
# OEC hyperparameters:
...
def main(args):
    ...
    model = BaselineFinetune(model.encoder, args['data.way'],args['data.shot'],loss_type='dist')
    ...
    def evaluate(data):
        ...
        for _ in tqdm(range(args['data.test_episodes'])):
            sample = load_episode(data, test_tr, args['data.test_way'], args['data.test_shot'], args['data.test_query'], device)
